var LichessPgnViewer = (function () {
    ...
    function applyAnimation(state, config) {
        if (config.animation) {
            deepMerge$1(state.animation, config.animation);
    ...
    function deepMerge$1(base, extend) {
        for (const key in extend) {
            if (Object.prototype.hasOwnProperty.call(extend, key)) {
                if (Object.prototype.hasOwnProperty.call(base, key) && isPlainObject$1(base[key]) && isPlainObject$1(extend[key]))
                    deepMerge$1(base[key], extend[key]);
                else
                    base[key] = extend[key];
            }
        }
    ...
    const renderControls = (ctrl) => h('div.lpv__controls', [
        ...
        h('button.lpv__fbt.lpv__controls__menu.icon', {
            class: {
    ...
    const dirButton = (ctrl, to, icon) => h(`button.lpv__controls__goto.lpv__controls__goto--${to}.lpv__fbt.icon.icon-${icon}`, {
    ...
    function view(ctrl) {
        const opts = ctrl.opts, staticClasses = `lpv.lpv--moves-${opts.showMoves}${opts.classes ? '.' + opts.classes.replace(' ', '.') : ''}`;
